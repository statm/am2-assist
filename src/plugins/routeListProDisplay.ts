...
function extractInfo(): [Array<HubInfo>, Array<RouteInfo>] {
    ...
    hubBoxes.each(function () {
        ...
        info.name = hub.find('div.hubListBoxTitle > i').get(0).previousSibling!.nodeValue!.trim().split(' ')[0];
        info.flag = hub.find('div.hubListBoxTitle > img').last().get(0).outerHTML;
    ...
    routeBoxes.each(function () {
        ...
        info.to = box.find('div.title > span.grey').get(0).nextSibling!.nodeValue!.replace('/', '').trim().split(' ')[0];
        ...
        info.remainingOffer = box.find('li.li2 b').text().trim();
            ...
            <tr${index % 2 === 1 ? ' class="rowB"' : ''}>
                <td class="tableString">
                    <div class="fixedwidth25 fll">
                        ${hub.isShared ? '<img src="/images/icons20/alliance_hubSharing.png?v1.6.11">' : '&nbsp;'}
                    </div>
                    <div class="fll">
                        ${hub.name} / ${hub.flag}
                    </div>
                ...
                <td>
                    <a href="${hub.detailsUrl}" title="Hub details"><img src="/images/icons20/network_showHub.png?v1.6.11"></a>
                </td>
            </tr>
        `);
        hubTable.append(row);
    ...
    routes.forEach((route: RouteInfo, index: number) => {
        const row = $(`
            <tr${index % 2 === 1 ? ' class="rowB"' : ''}>
                <td class="tableString">
                    <div class="fixedwidth25 fll">
                        ${route.isShared ? '<img src="/images/icons20/alliance_hubSharing.png?v1.6.11">' : '&nbsp;'}
                    </div>
                ...
                <td>
                    <a href="${route.detailsUrl}" title="Route details"><img src="/images/icons20/network_showLine.png?v1.6.11"></a>
                </td>
            </tr>
        `);
