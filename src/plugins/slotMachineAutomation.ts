...
export const slotMachineAutomation: Plugin = {
  ...
  action: function() {
    ...
    const logArea = $(
      '<textarea style="width:710px;height:100px;margin:25px 15px 0px 15px" readonly />',
    );
      ...
      $.get('cockpitasous/play', function(resp) {
        ...
        if (data.gain) {
          if (data.gain.gainLabel.endsWith(' R$')) {
            data.gain.gainLabel =
              '+R$ ' + data.gain.gainLabel.substr(1, data.gain.gainLabel.length - 3);
          } else if (data.gain.gainLabel.endsWith(' $')) {
            data.gain.gainLabel =
              '+$ ' + data.gain.gainLabel.substr(1, data.gain.gainLabel.length - 2);
          }
