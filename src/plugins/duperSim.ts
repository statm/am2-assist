...
function duperSimMain() {
        ...
        <div id="duperSimBox" class="secretaryBox" style="margin-top:13px;width:715px;min-height:128px">
            ...
            <div class="explanation">
                ...
                <input type="button" id="duperSimButton" class="validBtn validBtnBlue"
                    value="Perform a DUPER simulation with ${(
                      simulationCost * 5
                    ).toLocaleString()} $"
  ...
  $('#duperSimButton').click(async function() {
    ...
    const getPaxTextClass = (pax: number, seat: number) =>
      pax - supplies[seat] >= 0 ? 'num-pos' : 'num-neg';
    ...
    $('#row-supply, #row-price-step, #row-near-samples, #row-arrow-2, #row-pax-2').show(
      ANIMATION_SPEED,
    );
    ...
    for (const seat of [ECO, BUS, FIRST]) {
      const a1 =
        (sim[L1][seat].pax - sim[L2][seat].pax) / (simPrices[L1][seat] - simPrices[L2][seat]);
      const b1 = sim[L1][seat].pax - a1 * simPrices[L1][seat];
      const a2 =
        (sim[R1][seat].pax - sim[R2][seat].pax) / (simPrices[R1][seat] - simPrices[R2][seat]);
