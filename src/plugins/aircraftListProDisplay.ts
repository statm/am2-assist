...
function extractInfo(): Array<AirCraftInfo> {
  ...
  aircraftBoxes.each(function() {
    ...
    info.isCargo =
      box.find(
        'div.title span.greenBtn:contains("Cargo"), div.title span.cargoBtn:contains("Cargo")',
      ).length === 1;
    info.isLease = box.find('div.title span.greenBtn:contains("Lease")').length === 1;
    info.type = box
      .find('div.title span.editAircraftName')
      .get(0)
...
function constructTable(list: Array<AirCraftInfo>): JQuery<HTMLElement> {
  ...
  result.find('img.zoomAircraft').hover(
    function(event) {
      const src = $(this).data('aircraftimg');
      if (
        !$(this)
          .parents('table')
          .find('.zoom-aircraft').length
      ) {
        $(this)
          .parents('table')
          .prepend(
            `<img class="zoom-aircraft" src="${src}" style="position:absolute;top:${event.pageY -
              65}px;left:${event.pageX + 150}px;" />`,
          );
      }
    },
    function() {
...
function addCapacityInfo() {
  ...
  $('table.aircraftListViewTable tbody tr:not(:first-child)').each(function() {
    ...
    const aircraftTypeName = row
      .find('td.tableString b span.editAircraftName')
      .get(0)
      .previousSibling!.nodeValue!.replace('/', '')
      .trim();

    // Update seat number box
    let updatedText = seatBox.text();
    const aircraftData = AIRCRAFT_TABLE.find(
      (data: AircraftInfo) => data.name === aircraftTypeName,
    );
...
export const aircraftListProDisplay: Plugin = {
  ...
  action: async function() {
    ...
    if (!isInProDisplay) {
      ...
      $('div.aircraftListView')
        .empty()
        .prepend(proDisplayTable);
    }
