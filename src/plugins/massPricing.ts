...
function resetStatusIcons() {
    ...
    $('td.reliability').next().each(function() {
        const lineId = $(this).find('a').attr('href')!.split('/')[3];
        const statusIcon = $(`<img id="mpStatus${lineId}" class="mpStatusIcon" src="//i.imgur.com/pDMku5j.gif" width="16" height="16"/>`);
        $(this)
...
async function startMassPricing() {
    ...
    for (const line of linesToScan) {
        ...
        const priceUpdateResp = await fetch(`/marketing/pricing/${line}`, { method: 'POST', body: priceUpdateFormData });
    ...
    $('#mpStatusText').html(`Old turnover: <b>${currentTurnover.toLocaleString()} $</b>, new turnover: <b>${newTurnover.toLocaleString()} $</b>`);

    function updateStatus(line: string, successful: boolean) {
        ++total;
        $('#mpStatusText').html(`Scanning: <b>${total}/${linesToScan.length}</b>`);
        $(`#mpStatus${line}`).attr('src', successful ? '//i.imgur.com/5A3tGvf.png' : '//i.imgur.com/9jIoF80.jpg');
    }
}
