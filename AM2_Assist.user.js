...
(function() {
    ...
    define([".*"], function() {
        window.addEventListener("keyup", async function(event) {
            ...
            if (event.key == "?" && !event.altKey && event.ctrlKey && event.shiftKey) {
                ...
                for (const [index, aircraft] of aircraftInfo.entries()) {
                    const currentAircraft = AIRCRAFT_INFO[index];
                    assert(Object.keys(aircraft).every(key => {
                        return aircraft[key] == currentAircraft[key];
                    }));
                }
            }
        });
    ...
    async function loadAircraftInfo() {
        const aircraftInfo = [];
    ...
    for (const k in modules) {
        if (pageUrl.match(new RegExp(`^${k}$`))) {
            for (const funcPair of modules[k]) {
                console.log(`Running module: ${funcPair[1]}`);
                funcPair[0](k);
            }
        }
    }
